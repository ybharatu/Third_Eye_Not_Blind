#################################################################
# Library of still image processing functions
#################################################################
...
def handle_images(input_buffers, output_buffers, vid, filename, live, im, save):
    ...
    while (in_imgs < NUM_FRAMES or out_imgs < NUM_FRAMES):
        #################################################################
        # Code to handle single image processing
        # (NUM_FRAMES times for timing)
        #################################################################
        if (in_imgs != NUM_FRAMES):
            # code to process NUM_FRAMES images
            image = mpimg.imread(filename)
            #print("Input Images: " + str(in_imgs))
            #################################################################
        ...
        #################################################################
        # Dequeues a drift value from the current output buffer &
        # updates current output buffer
        #################################################################
        ...
        # right drift = 1, no drift = 0, no lines detected = 3
        #################################################################
        if (drift_value == -1):
            right_drift_cnt = 0
            left_drift_cnt += 1
        ...
        #################################################################
        # Increment output image count
        #################################################################
        #print("Output Images completed: " + str(out_imgs));
        out_imgs += 1
